...
_vpcalc.prototype = {
	renderData: function () {
		...
		$.each(data.ZoneList, function (index, value) {
			...
			if (data.ZoneList[index].status) {
				...
				var price = '<span style="display:block" class="zoneprice" id="zonePrice' + index + '">' + data.ZoneList[index].price + '</span>';
				var totalPrice = ' $ <span id="totalPrice' + index + '">0 </span> ';
				zonehtm = zonehtm + "<div id= 'zonename" + index + "' title= '" + data.ZoneList[index].title + "'>" + name + " <br />" + price + "<br / >" + totalPrice + "</div>";
	...
	addToZone: function (el) {
		...
		$("#zoneResult").find('[id^="zonename"]').each(function (index, value) {
			var totalPrice = parseInt($(this).find('[id^="totalPrice"').html());
			var zonePrice = parseInt($(this).find('[id^="zonePrice"').html());
			var newTotalPrice = ((parseFloat(el) * 100) * zonePrice) + totalPrice;
			$(this).find('[id^=totalPrice]').html(newTotalPrice)
	...
	removeFromZone: function (el) {
		...
		$("#zoneResult").find('[id^="zonename"]').each(function (index, value) {
			var totalPrice = parseInt($(this).find('[id^="totalPrice"').html());
			var zonePrice = parseInt($(this).find('[id^="zonePrice"').html());
			var newTotalPrice = totalPrice- ((parseFloat(el) * 100) * zonePrice) ;
			$(this).find('[id^=totalPrice]').html(newTotalPrice)
